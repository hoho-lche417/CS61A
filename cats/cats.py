...
def accuracy(typed: str, source: str) -> float:
    ...
    count_mismatch, min_len = 0, min(len_typed, len_source)
    for i in range(min_len):
        if typed_words[i] != source_words[i]:
            count_mismatch += 1
    if (len_typed > len_source):
        count_mismatch += (len_typed - len_source)
    return 100.0 - count_mismatch*100.0 / len_typed
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    min_diff = min(diff)
    idx_min_diff = diff.index(min_diff)
    if min_diff > limit:
        return typed_word
    return word_list[idx_min_diff]
    # END PROBLEM 5
...
def furry_fixes(typed: str, source: str, limit: int) -> int:
    ...
    cur_pos_unequal = typed[0] != source[0]
    return furry_fixes(typed[1:], source[1:], limit - int(cur_pos_unequal)) + int(cur_pos_unequal)
    # END PROBLEM 6
...
def minimum_mewtations(typed: str, source: str, limit: int) -> int:
    ...
    # Base cases
    if limit < 0:  
        return 1 # could use 0 as well
    len_typed, len_source = len(typed), len(source)
    if len_typed + len_source == 0: # if both empty string
        return 0
    elif len_typed * len_source == 0: # if one empty and another not empty
        return max(len_typed, len_source)
    
    # Recursive cases
    if typed[0] == source[0]: # Feel free 
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    for i_word in word_indices:
        min_i_player = 0
        min_time = get_time(times, 0, i_word)
        for i_player in player_indices:
            time = get_time(times, i_player, i_word)
            if time < min_time:
                min_time = time
                min_i_player = i_player
        fastest[min_i_player].append(words[i_word])
    return fastest
    # END PROBLEM 10
