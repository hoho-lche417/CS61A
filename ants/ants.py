...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        p, dist = self.place, 0
        while (p and not p.bees) or (p and dist < self.lower_bound):
            p = p.entrance
            dist += 1
        if p and not p.is_hive and dist <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        tot_damage = min(self.health, amount) + (self.damage if self.health <= amount else 0)
        bees = list(self.place.bees)
        for i in range(len(bees)):
            bees[i].reduce_health(tot_damage)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        bees = list(self.place.bees)
        for i in range(len(bees)):
            bees[i].reduce_health(self.damage)
# END Problem 9
