...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    num_fact = num_factors(score)
    if num_fact == 3 or num_fact == 4:
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    player_score, opponent_score, num_roll = 0, 0, strategy(0, 0)
    while player_score < goal:
        while opponent_score < goal:
            new_score = strategy(player_score, opponent_score)
            if new_score != num_roll:
                return False
            opponent_score += 1
        player_score += 1
        opponent_score = 0
...
def make_averaged(original_function, times_called=1000):
    ...
    def averaged_func(*args):
        ...
        while i < times_called:
            i, tot = i + 1, tot + original_function(*args)
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    max_avg_score, max_score_roll, i = 0, 0, 1
    while i <= 10:
        roll_dice_avg = make_averaged(roll_dice, times_called)
        avg_score = roll_dice_avg(i, dice)
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    #max_scoring_num_rolls(dice=six_sided, times_called=1000)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    if sus_update(0, score, opponent_score) - score >= threshold:
        return 0
    # score_boar_brawl = boar_brawl(score, opponent_score)
    # if sus_points(score_boar_brawl + score) - score >= threshold:
    #     return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
