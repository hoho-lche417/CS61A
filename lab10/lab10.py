...
def floor_div(args):
    ...
    result, divisor = args.first, args.rest
    while isinstance(divisor, Pair):
        result //= divisor.first
...
def eval_and(expressions):
    ...
    if expressions == nil:
        return True
    evaluated, exp = calc_eval(expressions.first), expressions.rest  
    while True: 
        if evaluated is scheme_f:
            return scheme_f
        if exp == nil:
            return evaluated
        evaluated, exp = calc_eval(exp.first), exp.rest
...
def eval_define(expressions):
    ...
    key, val = expressions.first, expressions.rest
    bindings[key] = calc_eval(val.first)
    return key
...
def print_evals(expr):
        ...
        *
        ...
        8
        """
        if not isinstance(expr, Pair): # base case
            if expr in ['+', '*']:
                print(expr)
