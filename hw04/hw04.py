def shuffle(s):
    ...
    shuffled = [0 for _ in range(len(s))]
    half_len = len(s) // 2
    for i in range(half_len):
        shuffled[2 * i] = s[i]
        shuffled[2 * i + 1] = s[i + half_len]
    return shuffled
...
def balanced(m):
    ...
    if is_planet(m):
        return True
    l, r = left(m), right(m)
    equal_torque = length(l) * total_mass(end(l)) == length(r) * total_mass(end(r))
    return equal_torque and balanced(end(l)) and balanced(end(r))
